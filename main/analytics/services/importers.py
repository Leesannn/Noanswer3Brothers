...
from analytics.models import (
    ApplicationStatus, DataSource, Institution, Program, ProgramMatchPayload,
    QualificationAggregate, UploadBatch,
)
from analytics.services.normalizers import (
    clean_text, normalize_institution, normalize_region, normalize_sport, normalized_key,
    split_qualification,
)
from analytics.services.sport_matching import match_payload_key
...
def _source_key(*values):
    ...
    # 128비트(32자리)면 현재 데이터 규모에서 충돌 가능성이 사실상 없고,
    # 64자리 SHA-256 원문보다 테이블과 고유 인덱스 크기를 절반으로 줄인다.
...
def _import_qualifications(rows, mapping, batch, errors, allow_invalid=False):
    ...
    from analytics.services.sport_matching import sync_taxonomy
    sync_taxonomy()
    ...
    if key not in cache:
        institution, _ = Institution.objects.update_or_create(
            ...
            defaults={'name': name_raw, 'region': region_raw, 'address': mapping_values.get('address', ''),
                      'institution_type': mapping_values.get('institution_type', '')},
        )
...
def _import_programs(rows, mapping, batch, errors, allow_invalid=False):
    ...
    payloads = {}
    valid_rows = 0
    source, _ = DataSource.objects.get_or_create(filename=batch.original_filename)
    ...
    for number, row in enumerate(rows, start=2):
        try:
            ...
            program = Program(
                ...
                capacity=_integer(values['capacity']), status=values['status'] or '운영', source=source,
            ...
            if matcher:
                ...
                payload_key = match_payload_key(result.rules, result.candidates)
                if payload_key not in payloads:
                    payloads[payload_key], _ = ProgramMatchPayload.objects.get_or_create(
                        payload_key=payload_key,
                        defaults={'rules': result.rules, 'candidates': result.candidates},
                    )
                program.match_payload = payloads[payload_key]
    ...
    Program.objects.bulk_create(
        ...
        update_fields=['institution', 'name', 'normalized_name', 'sport', 'normalized_sport', 'target', 'weekdays',
                       'start_time', 'end_time', 'start_date', 'end_date', 'capacity', 'status', 'source',
...
def _import_applications(rows, mapping, batch, errors, allow_invalid=False):
            ...
            if not program:
                ...
                program, _ = Program.objects.get_or_create(
                    ...
                    defaults={'institution': institution, 'name': values['program'], 'normalized_name': normalized_program,
                              ...
                              'capacity': _integer(values['capacity']), 'source': source},
                )
    ...
    ApplicationStatus.objects.bulk_create(
        ...
        update_fields=['program', 'reference_date', 'capacity', 'applicants', 'waitlist', 'is_synthetic'],
        unique_fields=['source_key'],
    )
