...
from analytics.models import ProgramCleanup
from analytics.services.sport_matching import SportMatcher, load_rules as load_sport_rules, matching_key
...
def cleanup_detail_key(evidence, rules):
    payload = json.dumps(
        {'evidence': evidence or '', 'rules': rules or []},
...
class ProgramCleaner:
    ...
    def operating_status(self, program, reference_date):
        ...
        if program.start_date and program.end_date:
            ...
            return ProgramCleanup.OperatingStatus.UPCOMING, f'시작일 {program.start_date}이 기준일 {reference_date}보다 이후입니다.', 'date_range_future'
    ...
    def clean(self, program, reference_date=None):
        ...
        elif status == ProgramCleanup.OperatingStatus.UPCOMING:
            exclusion_reason = ProgramCleanup.ExclusionReason.UPCOMING
        elif is_multiple:
