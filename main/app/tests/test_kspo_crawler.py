...
from app.models import ExamSchedule, ExamScheduleFetchStatus
from app.services import license_info
from app.services.kspo_client import KspoFetchError, fetch
from app.services.kspo_license import parse_license_html
from app.services.kspo_schedule import parse_schedule_html
...
class KspoClientTests(TestCase):
    @patch('app.services.kspo_client.requests.get')
...
class RefreshExamScheduleCommandTests(TestCase):
    def test_success_replaces_cache_and_clears_error(self):
        ...
        with patch('app.management.commands.refresh_exam_schedule.fetch_schedule', return_value=fresh_rows):
    ...
    def test_failure_preserves_existing_cache_and_records_error(self):
        ...
        with patch(
            'app.management.commands.refresh_exam_schedule.fetch_schedule',
    ...
    def test_empty_parse_result_is_treated_as_failure(self):
        ...
        with patch('app.management.commands.refresh_exam_schedule.fetch_schedule', return_value=[]):
            call_command('refresh_exam_schedule', '--grade', 'LSC2', stdout=StringIO(), stderr=StringIO())
